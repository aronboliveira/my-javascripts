...
function assignProductionLevel(sh, set) {
  ...
  if (!amRw || !ngRw) {
    Logger.log(
      "##ERRORCould not find both 'Amostra' and 'negativo' rows at##" + rpName
    );
    return;
  }
  ...
  if (lCls.length === 0 || !mnCl || !lvCl) {
    Logger.log(
      "##ERRORCould not find L-patterned columns, 'Média', or 'Nível' column.##" +
        " em " +
        rpName
    );
    return;
  ...
  Logger.log(
    "Valores capturados para Controle Negativo" +
      " em " +
      rpName +
      " : " +
      ngLValues
  ...
  if (ngLValues.length > 0) {
    ...
    Logger.log(
      "Média arimética de Controle Negativo" + " em " + rpName + " : " + ngMn
    );
    Logger.log(
      "Desvio Padrão de Controle Negativo " + " em " + rpName + " : " + sd
    );
    ...
    if (sdFlagCell) {
      ...
      values.forEach(function (v, i) {
        Logger.log(
          "Valor para Célula " +
            sh
              .getRange(sdFlagCell.getRow() - 1, sdFlagCell.getColumn() + i)
              .getA1Notation() +
            " em " +
            rpName +
            " : " +
  ...
  for (var row = amRw + 1; row < ngRw; row++) {
    ...
    if (lVls.length > 0) {
      ...
      Logger.log(
        "Nível na Célula " +
          sh.getRange(row, lvCl).getA1Notation() +
          " em " +
          rpName +
          " : " +
          level
...
function findRowContaining(sh, st) {
  ...
  for (var row = 0; row < dt.length; row++) {
    for (var col = 0; col < dt[row].length; col++) {
      if (
        typeof dt[row][col] === "string" &&
        dt[row][col].toLowerCase() === st
      ) {
        return row + 1;
      }
    }
...
function freezeReadOnlyData(sh) {
  ...
  Logger.log(
    "Faixas da linha " +
      ...
      " a " +
      ngRw +
      " em Colunas de Linhagens foram protegidas"
  );
}
  ...
  sh.getProtections(SpreadsheetApp.ProtectionType.RANGE).forEach(function (pr) {
    ...
    lCls.forEach(function (col) {
      if (
        ptRg.getColumn() === col &&
        ptRg.getRow() >= amRw + 1 &&
        ptRg.getLastRow() <= ngRw
      ) {
        pr.remove();
        Logger.log(
          "Unprotected range in column : " + col + ", from row " + (amRw + 1)
        );
      }
    });
  });
